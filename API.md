...
* `transport`: if you want to provide your own Transport object (from `safe-t-link` module)
...
| wipeDevice | force: ?boolean | Promise | Wipes the device (after user confirms).<br>If force is false it asks for PIN.<br>If force is true and device is locked, it will kill a PIN zone (see pin_remaining_zones) |
